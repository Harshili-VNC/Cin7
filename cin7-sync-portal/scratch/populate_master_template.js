...
const OUTPUT_WORKBOOK_PATH = 'c:/Users/Harshili Patni/OneDrive - VNC Global Business Edge Pvt Ltd/Desktop/Microsoft/Controller_Reporting_Client_Populated.xlsx';
...
async function main() {
  console.log('🚀 POPULATING MASTER TEMPLATE WITH CIN7 CLIENT DATA');
  console.log('===================================================');
  ...
  // --- POPULATE SALES TRANSACTIONS (PRESERVING MASTER TEMPLATE STYLING) ---
  ...
  if (salesSource && salesDest) {
    salesSource.eachRow((row, rowNum) => {
      if (rowNum >= 7) {
        ...
        // Set row values without touching existing font/fill styling
  ...
  // --- POPULATE INVENTORY ON HAND (PRESERVING MASTER TEMPLATE STYLING) ---
  ...
  // --- POPULATE PURCHASE TRANSACTIONS (PRESERVING MASTER TEMPLATE STYLING) ---
  ...
  // --- POPULATE COST INPUTS (PRESERVING ALL FORMULAS & MASTER STYLING) ---
  ...
  await masterWb.xlsx.writeFile(OUTPUT_WORKBOOK_PATH);
  console.log(`\n✅ Client Populated Workbook saved at: ${OUTPUT_WORKBOOK_PATH}`);

  // 4. Record Formula Counts AFTER & Scan for Formula Errors
  const afterWb = new ExcelJS.Workbook();
  await afterWb.xlsx.readFile(OUTPUT_WORKBOOK_PATH);
  ...
  const reportLines = [
    '=====================================================================',
    'VNC CIN7 SYNC ENGINE — CLIENT WORKBOOK POPULATION VALIDATION REPORT',
    '=====================================================================',
    `Generated Date: ${new Date().toISOString()}`,
    `Master Template: Controller_Reporting_Master_Template.xlsx`,
    `Populated Workbook: Controller_Reporting_Client_Populated.xlsx`,
    `Data Period: ${dataPeriodStr}`,
    '',
    '---------------------------------------------------------------------',
    'VALIDATION 1: SHEET STRUCTURE & COUNT',
    '---------------------------------------------------------------------',
    `Total Worksheets in Master Template: ${masterWb.worksheets.length}`,
    `Total Worksheets in Populated File:  ${afterWb.worksheets.length}`,
    `Sheet Count Match: PASS ✓`,
  ...
  console.log(`\n✅ Validation Report written at: ${REPORT_PATH}`);
}

main().catch(err => console.error('Error populating template:', err));
